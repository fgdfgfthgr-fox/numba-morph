...
class TestLMin2D:
    ...
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmin_2d_grey(self, connectivity, dtype, fscale, borders):
        ...
        result = local_minima(original, footprint=footprint, allow_borders=borders)
        expected = morphology.local_minima(original, footprint=footprint, allow_borders=borders).astype(np.bool_)
        np.testing.assert_array_equal(result, expected)

    @pytest.mark.parametrize("connectivity", [4, 8])
    @pytest.mark.parametrize("dtype", [np.uint8, np.int16])
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmin_2d_batched(self, connectivity, dtype, borders):
        ...
        for i in range(expected.shape[0]):
            expected[i] = morphology.local_minima(original[i], footprint=footprint, allow_borders=borders).astype(np.bool_)
        np.testing.assert_array_equal(result, expected)

    @pytest.mark.parametrize("connectivity", [4, 8])
    @pytest.mark.parametrize("dtype", [np.uint8, np.int16])
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmin_2d_coin(self, connectivity, dtype, borders):
        original = data.coins()
        footprint = connectivity_to_footprint(connectivity)
        result = local_minima(original, footprint=footprint, allow_borders=borders)
        expected = morphology.local_minima(original, footprint=footprint, allow_borders=borders).astype(np.bool_)
...
class TestHMin3D:
    ...
    @pytest.mark.parametrize("dtype", [np.uint8, np.int16])
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmin_3d_grey(self, connectivity, dtype, borders):
        original = np.random.randint(0, 128, (16, 32, 64), dtype=dtype) if not np.issubdtype(dtype, np.floating) else np.random.random((16, 32, 64)).astype(dtype)
        footprint = connectivity_to_footprint(connectivity)
        result = local_minima(original, footprint=footprint, allow_borders=borders)
        expected = morphology.local_minima(original, footprint=footprint, allow_borders=borders).astype(np.bool_)
    ...
    @pytest.mark.parametrize("dtype", [np.uint8, np.int16])
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmin_3d_batched(self, connectivity, dtype, borders):
...
class TestLMax2D:
    ...
    @pytest.mark.parametrize("dtype", [np.uint8, np.int16])
    @pytest.mark.parametrize("borders", [True, False])
    def test_lmax_2d_grey(self, connectivity, dtype, borders):
        ...
        footprint = connectivity_to_footprint(connectivity)
        result = local_maxima(original, footprint=footprint, allow_borders=borders)
        expected = morphology.local_maxima(original, footprint=footprint, allow_borders=borders).astype(np.bool_)
        np.testing.assert_array_equal(result, expected)