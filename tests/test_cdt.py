...
class TestDistanceTransform2D:
    ...
    def test_chamfer_distance_transform_2d(self, speed):
        original = np.random.randint(0, 2, (32, 64), dtype=np.uint8)
        result = distance_transform_cdt(original, weights=(1,2), speed=speed)
        expected = ndimage.distance_transform_cdt(original, 'taxicab')
        np.testing.assert_array_equal(result, expected)

    @pytest.mark.parametrize("speed", [True, False])
    def test_chamfer_distance_transform_2d_batched(self, speed):
...
class TestDistanceTransform3D:
    ...
    @pytest.mark.parametrize("speed", [True, False])
    def test_chamfer_distance_transform_3d(self, speed):
        original = np.random.randint(0, 2, (16, 32, 64), dtype=np.uint8)
        result = distance_transform_cdt(original, weights=(1,2,3), speed=speed)
        expected = ndimage.distance_transform_cdt(original, 'taxicab')
        np.testing.assert_array_equal(result, expected)

    @pytest.mark.parametrize("speed", [True, False])
    def test_chamfer_distance_transform_3d_batched(self, speed):
